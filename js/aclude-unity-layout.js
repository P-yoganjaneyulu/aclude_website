...
function initTypewriterEffect() {
    ...
    let texts = [originalText, alternativeText];
    let index = 0;

    // Ensure we can fade the whole heading
    heroText.style.transition = 'opacity 0.45s ease';
    ...
    function showAndScheduleNext() {
        ...
        typeWriter(current, () => {
            // After the full text is typed, wait then fade out whole element
            setTimeout(() => {
                heroText.style.opacity = '0';
                // After fade completes, switch to next text and fade back in while typing
                setTimeout(() => {
                    index = (index + 1) % texts.length;
                    heroText.textContent = '';
                    heroText.style.opacity = '1';
                    // small pause before typing the next text
                    setTimeout(showAndScheduleNext, 200);
                }, 500);
            }, 1200);
        });
    }

    // Kick off after a short delay so page animations finish
    setTimeout(() => {
        heroText.style.opacity = '1';
        showAndScheduleNext();
    }, 800);
